...
void *receive_packets(void *ptr)
{
    ...
    while (1)
    {
        ...
        if (prev_pkt_needs_elaborating)
        {
            pthread_cond_wait(&is_empty, &packet_lock);
        }

        received_packet = *pkt;
        prev_pkt_needs_elaborating = 1;
        free(pkt);
...
void game_setup(char *player_nickname, struct sockaddr_in tcp_server_address)
{
    ...
    tcp_player_client_sock = connect_to_tcp_server(tcp_server_address);
